...
Conditional_Log_Likelihood_Parameter <- function(theta, series_ts, controls){
  ...
  for (i in 3:n) {
    ...
    if (i == 3) {
      ...
      zeta[i, ] <- zeta[i, ]/sum(zeta[i, ]) # Normalise to avoid >1 probabilities
    }
  }
  ...
  for(i in (n-1):3){
    ...
    smooth_prob[i, ] <- smooth_prob[i, ]/sum(smooth_prob[i, ]) # Normalise to avoid pr > 1
  }
